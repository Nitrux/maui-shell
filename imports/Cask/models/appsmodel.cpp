    ...
    for (KServiceGroup::List::ConstIterator it = list.constBegin(); it != list.constEnd(); it++) {
        ...
        if (p->isType(KST_KServiceGroup)) {
            ...
            if (!s->noDisplay() && s->childCount() > 0) {
                ...
                res << FMH::MODEL {{FMH::MODEL_KEY::EXECUTABLE, KService(  FMH::PATHTYPE_URI[FMH::PATHTYPE_KEY::APPS_PATH] + s->entryPath()).exec()}, {FMH::MODEL_KEY::ICON, s->icon()}, {FMH::MODEL_KEY::LABEL, s->name()}, {FMH::MODEL_KEY::PATH, FMH::PATHTYPE_URI[FMH::PATHTYPE_KEY::APPS_PATH] + s->entryPath()}};
